...
d3.json('nyc.geojson').then(
    (data)=>{
        ...
        d3.json("https://gbfs.citibikenyc.com/gbfs/en/station_information.json").then((citiBikeData)=>{
            console.log(citiBikeData);

            // get the stations:
            let stations = citiBikeData.data.stations;

            // make an array to hold the bike markers
            let bikeMarkers = [];

            // loop through the array of stations
            for(let i = 0; i < stations.length; i++)
            {
                // for each station make a marker and add a popup with the station's name
                let bikeMarker = L.marker([stations[i].lat, stations[i].lon])
                        .bindPopup(`
                            <center>
                                <h3>${stations[i].name}</h3><hr>
                                <h4>Capacity: ${stations[i].capacity}</h4>
                            </center>
                            `);
            ...
            // make the layer group for the bike markers
            let bikeStations = L.layerGroup(bikeMarkers);

            // make overlay object
            let overlayMaps = {
                "Citi Bike Stations": bikeStations
            };
